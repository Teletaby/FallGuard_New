...
# Default pose detection settings (aligned with LSTM training configuration)
POSE_CONFIDENCE_DEFAULT = 0.25
POSE_IOU_DEFAULT = 0.45
...
def detect_multiple_people(image, mp_pose_instance=None, use_hog=False, max_people=None, conf=None, iou=None):
    """Detect up to a single person using YOLOv11n-Pose."""
    h, w, _ = image.shape
    
    if h < 100 or w < 100 or YOLO_MODEL is None:
        return []
    
    conf_threshold = conf if conf is not None else POSE_CONFIDENCE_DEFAULT
    iou_threshold = iou if iou is not None else POSE_IOU_DEFAULT
    # Force single-person detection regardless of supplied max_people.
    person_limit = 1
    
    try:
        # Single fast detection - optimized for speed
        results = YOLO_MODEL(image, conf=conf_threshold, iou=iou_threshold, verbose=False, half=False)
                    ...
                    try:
                        ...
                        for kpt in kpts:
                            if len(kpt) >= 3:
                                ...
                                if conf_kpt > KEYPOINT_CONFIDENCE_THRESHOLD and (0 <= x < w) and (0 <= y < h):
                                    valid_keypoints.append([x, y, conf_kpt])
                        
                        # Need at least 5 keypoints
                        if len(valid_keypoints) >= MIN_CONFIDENT_KEYPOINTS:
...
def build_pose_feature_vector(keypoints, frame_width, frame_height, min_valid_points=MIN_CONFIDENT_KEYPOINTS):
    """Flatten YOLO keypoints into (x, y, confidence) triples normalized by frame size."""
    if keypoints is None or len(keypoints) == 0 or frame_width <= 0 or frame_height <= 0:
        return None
    ...
    valid_points = 0
    joints = min(len(kpts), FEATURE_SIZE // 3)
    
    for idx in range(joints):
        base = idx * 3
        if len(kpts[idx]) < 3:
            continue
        x_px, y_px, conf = float(kpts[idx][0]), float(kpts[idx][1]), float(kpts[idx][2])
        if 0 <= x_px <= frame_width:
            feature_vec[base] = np.clip(x_px / frame_width, 0.0, 1.0)
        if 0 <= y_px <= frame_height:
            feature_vec[base + 1] = np.clip(y_px / frame_height, 0.0, 1.0)
        feature_vec[base + 2] = np.clip(conf, 0.0, 1.0)
...
def extract_8_kinematic_features(keypoints, frame_width=640, frame_height=480):
    ...
    try:
        ...
        if (
            l_shoulder_x is None or r_shoulder_x is None or
            l_shoulder_y is None or r_shoulder_y is None or
            l_hip_x is None or r_hip_x is None or
            l_hip_y is None or r_hip_y is None
        ):
            return np.zeros(8, dtype=np.float32)
...
def predict_torch(model, input_tensor, threshold=0.75):
    """Run LSTM inference returning (is_fall, probability)."""
    if model is None:
        return False, 0.0
    ...
    try:
        ...
        if output.ndim == 2 and output.shape[1] > 1:
            ...
            prob = torch.sigmoid(output).flatten().tolist()[0]
