...
LOG_LEVEL = os.environ.get("FALLGUARD_LOG_LEVEL", "INFO").upper()
logging.basicConfig(level=LOG_LEVEL, format="[%(levelname)s] %(message)s")
logger = logging.getLogger("fallguard")
AUTO_START_WEBCAM = os.environ.get("FALLGUARD_AUTOSTART_WEBCAM", "0").lower() in ("1", "true", "yes", "on")
LOG_FRAME_EVENTS = os.environ.get("FALLGUARD_LOG_FRAMES", "0").lower() in ("1", "true", "yes", "on")

def log_frame_debug(message):
    if LOG_FRAME_EVENTS:
...
logger.info(f"Initializing PyTorch. Using device: {device}")
...
try:
    try:
        ...
        if os.path.exists(MODEL_FILE):
            ...
            logger.info(f"Features: {FEATURE_SIZE}, Hidden: {HIDDEN_SIZE}, Output: {OUTPUT_SIZE}")
        else:
            logger.warning(f"Model file not found: {MODEL_FILE}")
            LSTM_MODEL = None
    except RuntimeError as e:
        if "size mismatch" in str(e):
            logger.info("Model shape mismatch detected, trying legacy format (output_size=1)")
            ...
            if os.path.exists(MODEL_FILE):
                ...
                LSTM_MODEL.to(device)
                LSTM_MODEL.eval()
                logger.info(f"Loaded LSTM Model (legacy format) from {MODEL_FILE}")
            else:
                LSTM_MODEL = None
        else:
            raise
    ...
    logger.info("Falling back to enhanced heuristic detection")
    LSTM_MODEL = None
...
class CameraProcessor(threading.Thread):
    ...
    def _match_person(self, bbox, threshold=100):
        ...
        if best_match is not None:
            log_frame_debug(f"[TRACKING] Matched Person #{best_match} (distance={best_distance:.1f}, score={best_score:.3f})")
    ...
    def update_camera_status(self, status, color, last_alert=None, is_live=True, person_count=0):
        with camera_lock:
            ...
            if person_count > 0:
                log_frame_debug(f"[STATUS] {self.name}: status='{status}', color={color}, people={person_count}, fps={self.current_fps:.1f}")
    ...
    def trigger_website_alert(self, fall_prob, person_id=None):
        ...
        logger.info(f"[ALERT] Website alert for {camera_display_name}: {fall_prob*100:.1f}% confidence (ID: {person_id})")
    ...
    def send_fall_alert_with_video(self, current_frame, fall_prob, person_id=None):
        ...
        if not TELEGRAM_BOT_TOKEN or not TELEGRAM_SUBSCRIBERS:
            logger.info("[TELEGRAM] No bot token or subscribers configured")
            return
                ...
                with telegram_lock:
                    for subscriber in TELEGRAM_SUBSCRIBERS:
                        try:
                            ...
                            if success:
                                logger.info(f"[TELEGRAM] Alert sent to {subscriber['name']} for {self.name} Person {person_id if person_id else '?'}")
                            else:
                                logger.warning(f"[TELEGRAM] Failed to send alert to {subscriber['name']}")
                        except Exception as e:
                            logger.error(f"[TELEGRAM] Failed to send alert to {subscriber['name']}: {e}")
            except Exception as e:
                logger.error(f"[TELEGRAM] Error in async alert sending: {e}")
    ...
    def predict_fall_enhanced(self):
        ...
        if len(self.pose_sequence) > 0 and self.latest_features is not None:
            ...
            if self.debug_counter % 30 == 0:
                log_frame_debug(f"[{self.name}] Heuristic: HWR={HWR:.2f}, Torso={TorsoAngle:.0f}°, H={H:.2f}, Angle={FallAngleD:.0f}°, Score={fall_probability:.2f}")
        ...
        if LSTM_MODEL is not None and len(self.pose_sequence) >= self.sequence_length:
            try:
                ...
                if prob > fall_probability:
                    ...
                    if self.debug_counter % 30 == 0:
                        log_frame_debug(f"[{self.name}] LSTM override: {prob:.2f}")
            except Exception as e:
                logger.error(f"LSTM prediction failed for {self.camera_id}: {e}")
    ...
    def draw_enhanced_overlay(self, frame, bbox, fall_confirmed, fall_prob, current_threshold, feature_vector):
        ...
        hide_overlays = bool(GLOBAL_SETTINGS.get('hide_overlays', False))
        
        if (not hide_overlays) and bbox is not None and bbox[2] > 0 and bbox[3] > 0:
            x, y, bw, bh = bbox
        ...
        if (not hide_overlays) and privacy_mode in ['full_video', 'skeleton_only'] and self.latest_pose_results:
    ...
    def run(self):
        ...
        logger.info(f"[{self.name}] Opening video source: {self.src}")
        
        max_retries = 3
        for attempt in range(max_retries):
            logger.info(f"[{self.name}] Attempt {attempt + 1}: Opening source: {self.src} (type: {type(self.src).__name__})")
            self.cap = cv2.VideoCapture(self.src)
            
            if self.cap and self.cap.isOpened():
                ret, test_frame = self.cap.read()
                if ret:
                    logger.info(f"Camera '{self.name}' opened on attempt {attempt + 1}")
                    break
                else:
                    self.cap.release()
                    self.cap = None
            ...
            if attempt < max_retries - 1:
                logger.warning(f"[RETRY] Attempt {attempt + 1} failed, retrying...")
                time.sleep(1)
        ...
        log_frame_debug(f"[INIT] {self.name}: Source type check: src={self.src}, is_str={isinstance(self.src, str)}, is_digit={str(self.src).isdigit()}, exists={os.path.exists(self.src) if isinstance(self.src, str) else 'N/A'}, is_video_file={is_video_file}")

        if not is_video_file:
            self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
            self.cap.set(cv2.CAP_PROP_FPS, 30)
            log_frame_debug(f"[INIT] {self.name}: Configured as live camera (buffersize=1, fps=30)")
        else:
            video_fps = self.cap.get(cv2.CAP_PROP_FPS)
            total_frames = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))
            log_frame_debug(f"[INIT] {self.name}: Configured as video file (FPS={video_fps}, Frames={total_frames})")
        ...
        if ret:
            ...
            log_frame_debug(f"[INIT] {self.name}: Read first frame successfully")
            
            if is_video_file:
                self.cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                log_frame_debug(f"[INIT] {self.name}: Reset video to frame 0 for processing loop")
        else:
            logger.error(f"[INIT] {self.name}: Could not read first frame")
        ...
        logger.info(f"Camera '{self.name}' started successfully with ID={self.camera_id}")
        ...
        logger.info(f"[START] {self.name}: Starting main loop with pose_process_interval={self.pose_process_interval}, force_detection_next_frame=True")
        ...
        try:
            while self.is_running:
                ...
                if is_video_file and not ret:
                    ...
                    logger.info(f"[VIDEO-LOOP] {self.name}: Video looped - cleared all trackers, reset person IDs, forcing detection")

                if not ret:
                    consecutive_failures += 1
                    if consecutive_failures >= max_failures:
                        logger.error(f"{self.name}: Too many consecutive failures")
                        break
                    time.sleep(0.05)
                    continue
                ...
                if self.mp_pose_instance and (self.force_detection_next_frame or (self.frame_count % max(1, self.pose_process_interval) == 0)):
                    ...
                    people = all_people
                    if people:
                        log_frame_debug(f"[DETECTION] Found {len(people)} person(s) in frame #{self.frame_count}")
                    else:
                        log_frame_debug(f"[DETECTION] No people detected in frame #{self.frame_count}")
                ...
                if len(people) > 0:
                    for person_idx, person in enumerate(people):
                        ...
                        if person_id is None:
                            ...
                            self.next_person_id += 1
                            logger.info(f"[DETECTION] New person detected: Person #{person_id}")
                        ...
                        try:
                            ...
                            feature_vec = np.zeros(FEATURE_SIZE, dtype=np.float32)
                            logger.error(f"Feature extraction for person {person_id}: {e}")
                        ...
                        if is_falling_person:
                            ...
                            if self.person_fall_states[person_id]['frames'] >= 3:
                                ...
                                logger.warning(f"[FALL] Person #{person_id} at {self.name} - Confidence: {fall_prob_person*100:.1f}%")
                        else:
                ...
                for person_id in stale_people:
                    ...
                    log_frame_debug(f"[TRACKING] Person #{person_id} removed from {self.name} (timeout)")
                        ...
                        if person_id in self.people_trackers:
                            ...
                            if pose_landmarks:
                                ...
                                try:
                                    ...
                                    log_frame_debug(f"[SKELETON] Drew skeleton for person #{person_id} (falling={person_falling})")
                                except Exception as e:
                                    logger.error(f"[SKELETON] Failed to draw skeleton: {e}")
                    ...
                    for person_idx, person_id in enumerate(sorted(tracked_people_ids)):
                        if person_id in self.people_trackers:
                            ...
                            if self.frame_count % 30 == 0:
                                log_frame_debug(f"[DETECTION] Person #{person_id} at ({x},{y}) size={x2-x}x{y2-y}, falling={person_falling}")
            ...
            logger.error(f"Camera processor crashed: {e}")
            logger.error(f"Camera: {self.name} (ID: {self.camera_id})")
            import traceback
            traceback.print_exc()
        finally:
            try:
            ...
            logger.info(f"Camera '{self.name}' (ID: {self.camera_id}) stopped")
    ...
    def create_incident_report(self, frame, fall_probability, person_id=None):
        ...
        logger.info(f"[INCIDENT] Created report {incident_id} for fall at {timestamp} - {person_label}")

# MJPEG Stream Generator
def generate_mjpeg(camera_id):
    boundary = b'--frame\r\n'
    
    log_frame_debug(f"[MJPEG] Starting stream for camera_id='{camera_id}'")
    log_frame_debug("[MJPEG] Looking for camera_id in shared_frames...")
    
    wait_time = 0
    max_wait = 5
    
    while camera_id not in shared_frames and wait_time < max_wait:
        log_frame_debug(f"[MJPEG] Waiting... ({wait_time}s/{max_wait}s) Cameras available: {list(shared_frames.keys())}")
        time.sleep(0.1)
        wait_time += 0.1
    
    if camera_id not in shared_frames:
        logger.warning(f"[MJPEG] Camera {camera_id} not found in shared_frames after {max_wait}s")
        log_frame_debug(f"[MJPEG] Available cameras: {list(shared_frames.keys())}")
        placeholder = 100 * np.ones((480, 640, 3), dtype=np.uint8)
    ...
    log_frame_debug(f"[MJPEG] Found camera {camera_id}, starting stream...")
    frame_count = 0
    while camera_id in shared_frames:
        ...
        yield boundary + b'Content-Type: image/jpeg\r\n\r\n' + frame_bytes + b'\r\n'
        if frame_count % 30 == 0:
            log_frame_debug(f"[MJPEG] Streaming {camera_id}: frame {frame_count}")
...
def video_feed(camera_id):
    log_frame_debug(f"[VIDEO_FEED] Request for camera_id='{camera_id}', type={type(camera_id)}")
    log_frame_debug(f"[VIDEO_FEED] Available cameras in shared_frames: {list(shared_frames.keys())}")
    log_frame_debug(f"[VIDEO_FEED] Available cameras in CAMERA_DEFINITIONS: {list(CAMERA_DEFINITIONS.keys())}")
...
def snapshot(camera_id):
    ...
    if camera_id not in shared_frames:
        log_frame_debug(f"[SNAPSHOT] Camera {camera_id} not in shared_frames after {max_wait}s wait")
        placeholder = 100 * np.ones((480, 640, 3), dtype=np.uint8)
    ...
    if frame is None:
        log_frame_debug(f"[SNAPSHOT] Camera {camera_id} has no frame yet")
        placeholder = 100 * np.ones((480, 640, 3), dtype=np.uint8)
...
def api_settings():
    ...
    if request.method == 'POST':
        ...
        new_hide_overlays = data.get('hide_overlays')
        if new_hide_overlays is not None:
            if isinstance(new_hide_overlays, bool):
                GLOBAL_SETTINGS['hide_overlays'] = new_hide_overlays
            elif isinstance(new_hide_overlays, (int, float)):
                GLOBAL_SETTINGS['hide_overlays'] = bool(new_hide_overlays)
            elif isinstance(new_hide_overlays, str):
                GLOBAL_SETTINGS['hide_overlays'] = new_hide_overlays.strip().lower() in ("1", "true", "yes", "on")
            else:
                GLOBAL_SETTINGS['hide_overlays'] = bool(new_hide_overlays)
            message.append("Overlay drawing updated")
...
if __name__ == '__main__':
    ...
    if AUTO_START_WEBCAM:
        ...
        print("[STARTUP] Default webcam auto-start disabled")
