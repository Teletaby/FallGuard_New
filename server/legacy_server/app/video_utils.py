        ...
        if full_results and full_results.pose_landmarks:
            ...
            if len(visible_landmarks) >= 1:
                ...
                if bw > 15 and bh > 25:
                    ...
                    if LOG_FRAME_EVENTS:
                        logger.debug(f"[DETECTION] Found person #{len(people)} at ({bx}, {by}) size={bw}x{bh}, conf={conf:.2f}")
        
        # Sort by area (largest first)
        people.sort(key=lambda p: p['area'], reverse=True)
        
    except Exception as e:
        logger.error(f"Multi-person detection failed: {e}")
        pass
...
def predict_torch(model, sequence_tensor, threshold=0.5):
    ...
    try:
        ...
        logger.error(f"Error during torch prediction: {e}")
        return 0, 0.0